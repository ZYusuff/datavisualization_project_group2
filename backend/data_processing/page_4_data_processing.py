...
def load_all_sheets():
    ...
    if _cached_sheets is None:
        ...
        _cached_sheets = pd.read_excel(path, skiprows=3, sheet_name=None)
...
def filter_data_by_direction_and_year(df, direction, year):
    ...
    result = df_filtered[["Utbildningsområde", col_name]].copy()
    result.rename(columns={col_name: "value"}, inplace=True)
    return result


def prepare_funnel_data(direction, year):
...
def load_sheet_with_gender(sheet_df: pd.DataFrame) -> pd.DataFrame:
    df = sheet_df.copy()
    gender = "total"
    gender_col = []
